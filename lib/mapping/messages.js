...
function convertHomeBridgeValueToAliceValue(capability_data, characteristic_data) {
  ...
  switch (capability_data.type) {
    case "devices.capabilities.on_off":
      converted_capability_data.key = "on_off"; // sber feature
      converted_capability_data.value.type = "BOOL";
      if (characteristic_data.description == 'Target Door State' || characteristic_data.description == 'Lock Target State') {
        if (characteristic_data.value) {
          converted_capability_data.value.bool_value = false;
        } else {
          converted_capability_data.value.bool_value = true;
        }
    ...
    case "devices.capabilities.range":
      if (capability_data.parameters && capability_data.parameters.type) {
        converted_capability_data.value.type = capability_data.parameters.type;
        converted_capability_data.value.bool_value = characteristic_data.value;
        return converted_capability_data;
      }
      break;
    ...
    case "devices.capabilities.mode":
      if (capability_data.parameters && capability_data.parameters.type) {
        converted_capability_data.value.type = capability_data.parameters.type;
        if (capability_data.parameters.type == 'thermostat') {
          switch (characteristic_data.value) {
            case 3:
              converted_capability_data.value.bool_value = "auto";
              return converted_capability_data;

            case 2:
              converted_capability_data.value.bool_value = "cool";
              return converted_capability_data;
            case 1:
              converted_capability_data.value.bool_value = "heat";
              return converted_capability_data;
            case 0:
              converted_capability_data.value.bool_value = capability_data.parameters.modes[0].value;
              return converted_capability_data;
            default:
              break;
          }
        } else if (capability_data.parameters.type == 'fan_speed') {
          switch (true) {
            case (characteristic_data.value > 66):
              converted_capability_data.value.bool_value = "high";
              return converted_capability_data;
            case (characteristic_data.value < 33):
              converted_capability_data.value.bool_value = "low";
              return converted_capability_data;
            default:
              converted_capability_data.value.bool_value = "medium";
              return converted_capability_data;
              break;
          }
        }
    ...
    case "devices.capabilities.color_setting":
      if (capability_data.parameters && capability_data.parameters.color_model) {
        converted_capability_data.value.type = capability_data.parameters.color_model;
        converted_capability_data.value.bool_value = {};
        converted_capability_data.value.bool_value.v = 100;
        if (characteristic_data.description == "Hue")
          converted_capability_data.value.bool_value.h = characteristic_data.value;
        else if (characteristic_data.description == "Saturation")
          converted_capability_data.value.bool_value.s = characteristic_data.value;
        return converted_capability_data;
      } else if (capability_data.parameters && capability_data.parameters.temperature_k) {
        converted_capability_data.value.type = "temperature_k";
        converted_capability_data.value.bool_value = characteristic_data.value * 10;
        return converted_capability_data;
      }
      break;
    ...
    case "devices.properties.event":
      if (capability_data.parameters && capability_data.parameters.type) {
        converted_capability_data.value.type = capability_data.parameters.type;
        if (capability_data.parameters.type == "open")
          converted_capability_data.value.bool_value = characteristic_data.value ? "opened" : "closed";
        else if (capability_data.parameters.type == "water_leak")
          converted_capability_data.value.bool_value = characteristic_data.value ? "leak" : "dry";
        else if (capability_data.parameters.type == "motion" || capability_data.parameters.type == "smoke")
          converted_capability_data.value.bool_value = characteristic_data.value ? "detected" : "not_detected";
        else if (capability_data.parameters.type == "battery_level")
          converted_capability_data.value.bool_value = characteristic_data.value ? "low" : "normal";
        else if (capability_data.parameters.type == "button") {
          switch (characteristic_data.value) {
            case 2:
              converted_capability_data.value.bool_value = "long_press";
              return converted_capability_data;
            case 1:
              converted_capability_data.value.bool_value = "long_press";
              return converted_capability_data;
            case 0:
              converted_capability_data.value.bool_value = "click";
              return converted_capability_data;
            default:
              break;
          }
